...
public class PictureDBHelper extends SQLiteOpenHelper {
    ...
    public void addBitmap(byte [] image) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues dbImg = new ContentValues();
        dbImg.put(COLUMN_IMAGE, image);
        long result = db.insert(TABLE_PIC, null, dbImg);
        if(result == -1){
            Toast.makeText(context, "Failed Insert Image", Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(context, "Successfully! new image", Toast.LENGTH_SHORT).show();
        }
    }
    //Delete All Trip
    void deleteImages(){
